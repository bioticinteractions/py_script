...
"""
class fy_calendar:
    def __init__(self, date_start, date_stop, output = 'df'):
    ...
    def fiscal_calendar(self):
        ...
        num_days = self.stop - self.start
       
        # create list of dates between the two input dates
        fy_date_list = [self.start + timedelta(days = i) for i in xrange(num_days.days + 1)]
...
dan = fy_dates(2017)
test = dan.get_dates()
#print dan.fiscal_calendar().head()

df_years = DataFrame()
for i in xrange(2010, 2018+1):
    df_years = df_years.append(fy_dates(i).fiscal_calendar())
